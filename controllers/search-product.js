...
const loadProduct = async () => {
    try {
      ...
      if (textoBuscado === null) {
        console.log("Hubo un error al buscar el producto"); // Corregida la cadena de error
        return;
      }
      ...
      data.forEach(({ id, categoria, nombre, precio, imagen }) => {
        ...
        if (validarNombre || validarCategoria) {
          const productosBuscados = newProduct(
            id, categoria, nombre, precio, imagen
          );
  ...
  document.addEventListener("DOMContentLoaded", loadProduct); // Esperar a que el DOM esté listo antes de cargar los productos
  */